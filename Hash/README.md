...
The computer actually stores people's password in the following format
```
...
If passwords are in plaintext, hackers can actually catch them immediately.
...
For example, iPad unlock screen password allows six digits.
Each digit can be zero to nine.
How many possible passcodes can you have on iPad? 10<sup>6</sup>.
Let's assume the attacker gets your password hashes stored on your iPad.
How can the attacker crack password hashes to get your original password?
...
They use a password list which has popular passwords around the workd.
Such popular password list can be obtained from the Internet black market.
The attackers will try passwords in the password list first and see which one produces the same password hash.
This is called dictionary attack, which is often fast in seconds.
If the attacker cannot get the password,
